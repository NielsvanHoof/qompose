...
export default function DossierBuilderLayout({
    ...
    return (
        <AppShell variant="header" className="h-dvh min-h-0 overflow-hidden">
            <SkipToContent />
            {/* Fixed chrome — content below scrolls inside panes, not the page. */}
            <header className="z-20 shrink-0 border-b bg-background/95 backdrop-blur-sm">
                <div className="mx-auto flex h-14 w-full max-w-[100rem] items-center gap-3 px-4 md:px-6">
                    <Link
                        href={backHref}
                        className="inline-flex shrink-0 items-center gap-1.5 rounded-md px-2 py-1.5 text-sm text-muted-foreground transition-colors hover:bg-muted/60 hover:text-foreground focus-visible:ring-[3px] focus-visible:ring-ring/50 focus-visible:outline-none"
                    >
            ...
            <AppContent
                id="main-content"
                variant="header"
                className="mx-auto flex min-h-0 w-full max-w-[100rem] flex-1 flex-col overflow-hidden"
                tabIndex={-1}
            >
                {children}
            </AppContent>
